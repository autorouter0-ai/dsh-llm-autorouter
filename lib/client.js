window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region src/client/bind-snapshot.ts
		/**
		...
		*/
		function useStoreSelector(subscribe, getSnapshot, selector, isEqual) {
			const selected = selector((0, react.useSyncExternalStore)(subscribe, getSnapshot));
			const cached = (0, react.useRef)(selected);
...
`;
		...
		function ensureCardStyles() {
			if (typeof document === "undefined" || document.getElementById("dsh-llm-autorouter-card-css") !== null) return;
		...
		*/
		...
		function AutorouterCard({ scope, useSnapshot, api, t }) {
			...
			const resetField = () => {
				const inherited = typeof baseLayer?.baseURL === "string" && baseLayer.baseURL.length > 0 ? baseLayer.baseURL : DEFAULT_BASE_URL;
