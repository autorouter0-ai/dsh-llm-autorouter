...
* OpenAI/DeepSeek protocol: the literal `[DONE]` is yielded so the caller
...
*
* Adapted from `@deepseek-ai/dsh-llm-deepseek/src/sse.ts` (MIT).
*
* @module dsh-llm-autorouter/sse
*/
/** The terminal payload OpenAI-compatible relays send after the last chunk. */
const DONE = "[DONE]";
/**
...
var AutorouterAdapter = class extends LlmAdapter {
	...
	async *stream(options) {
		...
		using watchdog = idleWatchdog(options.signal === void 0 ? consumer.signal : AbortSignal.any([options.signal, consumer.signal]), connection.streamIdleTimeoutMs, STREAM_IDLE_TIMEOUT_CODE);
